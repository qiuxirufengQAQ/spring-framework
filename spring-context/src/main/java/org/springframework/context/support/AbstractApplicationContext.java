 ...
 * 不强制用于配置的存储类型；简单地实现公共上下文功能。使用模板方法设计模式，需要具体的子类来实现抽象方法。
 *
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 这些监听器是注册到ApplicationEventMulticaster中的。这不会影响到其它监听器bean。
				// 在注册完以后，还会将其前期的事件发布给相匹配的监听器。
				registerListeners();
	...
	protected void prepareRefresh() {
		...
		// 验证所有标记为必需的属性都是可解析的：请参阅ConfigurablePropertyResolver＃setRequiredProperties
